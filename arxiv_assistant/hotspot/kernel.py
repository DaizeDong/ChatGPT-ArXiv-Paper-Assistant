...
from arxiv_assistant.hotspot.enrich import (
...
def _stage_harvest(ctx: KernelContext) -> dict[str, Any]:
    ...
    try:
        from arxiv_assistant.hotspot.support.source_fetch import url_title_consistent
...
def _stage_date_verify(ctx: KernelContext) -> dict[str, Any]:
    ...
    try:
        from arxiv_assistant.hotspot.date_verify import verify  # type: ignore[import]
        for it in items:
            verdict = _with_retry(
...
def _stage_score(ctx: KernelContext) -> dict[str, Any]:
    ...
    try:
        from arxiv_assistant.hotspot.dedup import (  # type: ignore[import]
            classify_cross_day,
            cluster_intraday,
            match_crossday,
        )
        ...
        from arxiv_assistant.hotspot.story import apply_cross_day_penalty
    except ImportError:
...
def _call_synthesize_agent(topics: list[dict[str, Any]], model: str) -> dict[str, Any]:
    ...
    from arxiv_assistant.hotspot.synthesize import synthesize_bilingual  # stage-deferred impl
...
def _resurge(story, *, max_age_days, run_date, min_competitors, cooldown_days):
    ...
    try:
        from arxiv_assistant.hotspot.novelty import resurge  # type: ignore[import]
...
def _stage_delta(ctx: "KernelContext") -> dict[str, Any]:
    ...
    from arxiv_assistant.hotspot.support.config import repo_root
