...
1. [ABot-World-0: Infinite Interactive World Rollout on a Single Desktop GPU](#paper-2607-19191)
...
2. [Breaking Feedback-Blindness: Utility-Augmented Transformer for Sequential Decision Making](#paper-2607-18910)
**Authors:** Yuyang Shen, Shan Dai, Daimin Chen

3. [Soft-TransFormers for Continual Learning](#paper-2411-16073)
...
5. [QScheduler: Adaptive Gradient Sampling for Zeroth-Order On-Device Training on INT8 NPUs](#paper-2607-18802)
**Authors:** Victor Felipe Domingues Do Amaral, Pierre Demaj, Erwan Libessart, Laurent Folliot, Anthony Kolar, Philippe B̩nab̬s

6. [Stability of Low-Rank Implicit Regularization in Perturbed Deep Matrix Factorization](#paper-2605-28613)
**Authors:** Jingzhe Wang, Hung-Hsu Chou

7. [RELTA-SGLD: Relative-Growth Localized Taming for Nonconvex Stochastic-Gradient Langevin Learning](#paper-2607-19544)
...
10. [Finite-Agent Stochastic Differential Games on Large Graphs: II. Graph-Based Architectures](#paper-2509-12484)
**Authors:** Ruimeng Hu, Jihao Long, Haosheng Zhou

11. [LieBN: Batch Normalization over Lie Groups](#paper-2607-08783)
**Authors:** Ziheng Chen, Yue Song, Rui Wang, Xiao-Jun Wu, Nicu Sebe

12. [Spectral Higher-Order Neural Networks Have Sharp Expressivity Bounds](#paper-2607-19042)
**Authors:** Gianluca Peri, Diego Febbe, Duccio Fanelli

13. [RAMP: Recognition parametrisation by Amortised Message Passing](#paper-2607-18883)
...
1. [Scaling Laws for Hypernetwork-Based Knowledge Injection in Large Language Models](#paper-2607-19604)
...
3. [Contraction-Gauge Preconditioning for Quantized Matrix Multiplication](#paper-2607-18745)
**Authors:** Piyush Sao, Narasinga Miniskar, Pedro Valero-Lara, Keita Teranishi, Sudip Seal

4. [Chebyshev Manifold Adaptation](#paper-2607-17377)
**Authors:** Jiawen Li

5. [Visual Token Compression Enhances Robustness of MLLMs](#paper-2607-22716)
**Authors:** Shishen Gu, Jiequan Cui, Wenbo Hu, Zenglin Shi, Zhenzhen Hu, Richang Hong

6. [MoA-Structured Decode Attention DNF Derivation, KV-Cache Accumulation, GQA/MQA, and OpenACC Kernel](#paper-2607-19456)
...
### 1. [ABot-World-0: Infinite Interactive World Rollout on a Single Desktop GPU](https://arxiv.org/abs/2607.19191) <a id="paper-2607-19191"></a>
...
**Comment:** Combines causal distillation and extended teacher rollouts to stabilize an interactive video world model.

**Topic Match:** The technical-report format is the nearest fit; the model generates interactive video worlds.
...
**Comment:** Invariant relational parameterization removes redundant coordinates from transformer representations, attention, and optimization.

**Topic Match:** The core proposal replaces transformer representation and optimization primitives; empirical training scale is unspecified.
...
### 2. [Breaking Feedback-Blindness: Utility-Augmented Transformer for Sequential Decision Making](https://arxiv.org/abs/2607.18910) <a id="paper-2607-18910"></a>
...
**Comment:** A compact utility state lets action-reward history directly modulate attention retrieval.

**Topic Match:** Attention is redesigned for sequential decision policies in control, finance, and recommendation rather than language-model training.
...
### 3. [Soft-TransFormers for Continual Learning](https://arxiv.org/abs/2411.16073) <a id="paper-2411-16073"></a>
...
**Comment:** Task-specific attention masks preserve a shared frozen solution and constrain adaptation drift.

**Topic Match:** The structural training argument concerns parameter-efficient continual adaptation of vision transformers.
...
**Comment:** Varying consistency-loss weight tests when latent transition geometry becomes contractive.

**Topic Match:** The analysis concerns training geometry in video predictors and control world models, outside the specified subject.
...
### 5. [QScheduler: Adaptive Gradient Sampling for Zeroth-Order On-Device Training on INT8 NPUs](https://arxiv.org/abs/2607.18802) <a id="paper-2607-18802"></a>
...
**Comment:** Adapts the number of zeroth-order gradient samples to training progress, avoiding fixed-sample-count tuning.

**Topic Match:** The optimizer schedule is the nearest fit; the experiments train image classifiers on an INT8 NPU.
...
### 6. [Stability of Low-Rank Implicit Regularization in Perturbed Deep Matrix Factorization](https://arxiv.org/abs/2605.28613) <a id="paper-2605-28613"></a>
...
**Comment:** Spectral conditions explain when gradient descent preserves a low-rank phase under target perturbations.

**Topic Match:** Optimization dynamics are central, but the analyzed training model is deep matrix factorization rather than a language model.
...
### 7. [RELTA-SGLD: Relative-Growth Localized Taming for Nonconvex Stochastic-Gradient Langevin Learning](https://arxiv.org/abs/2607.19544) <a id="paper-2607-19544"></a>
...
**Comment:** Relative-growth taming preserves more of the original update while establishing stability for superlinear gradients.

**Topic Match:** The core subject is stochastic-gradient Langevin analysis, with Fashion-MNIST validation rather than language-model training.
...
**Comment:** Single-site conditional estimation replaces discrete score approximation in the reverse diffusion process.

**Topic Match:** The generative parameterization is architectural, but the trained models concern images and physical discrete systems rather than language.
...
**Comment:** Embedding known physical subexpressions in separate architectural levels reduces what the model must learn.

**Topic Match:** Hierarchical Fourier operators are trained for physical PDE dynamics, an explicitly excluded subject.
...
### 10. [Finite-Agent Stochastic Differential Games on Large Graphs: II. Graph-Based Architectures](https://arxiv.org/abs/2509.12484) <a id="paper-2509-12484"></a>
...
**Comment:** Embeds fixed graph-aligned components into feedforward networks to reduce trainable parameters.

**Topic Match:** Architectural sparsification is the nearest category; the trained networks solve stochastic differential games.
...
### 11. [LieBN: Batch Normalization over Lie Groups](https://arxiv.org/abs/2607.08783) <a id="paper-2607-08783"></a>
...
**Comment:** Invariant Lie-group metrics give normalization explicit control of manifold-valued means and variances.

**Topic Match:** Normalization design is the nearest topic, but the framework targets manifold-valued networks without an established language-model setting.
...
### 12. [Spectral Higher-Order Neural Networks Have Sharp Expressivity Bounds](https://arxiv.org/abs/2607.19042) <a id="paper-2607-19042"></a>
...
**Comment:** Spectral parameter sharing connects neural-hypergraph expressivity to constrained parameter growth.

**Topic Match:** The architectural analysis studies neural hypergraphs on parity tasks, without a language-model training setting.
...
### 13. [RAMP: Recognition parametrisation by Amortised Message Passing](https://arxiv.org/abs/2607.18883) <a id="paper-2607-18883"></a>
...
**Comment:** Learns nonlinear message-passing rules that implicitly define latent-variable structure.

**Topic Match:** Closest to parameterisation design, with latent-variable inference as the core subject.
...
### 1. [Scaling Laws for Hypernetwork-Based Knowledge Injection in Large Language Models](https://arxiv.org/abs/2607.19604) <a id="paper-2607-19604"></a>
...
**Comment:** Separating hypernetwork injection capacity from target-model size exposes independent adaptation-scaling axes.

**Topic Match:** Scaling laws are the closest fit, but they configure LoRA-based factual knowledge injection rather than foundational model training.
...
**Comment:** On-policy distillation targets drafting variance associated with bidirectional diffusion attention.

**Topic Match:** Training improves an auxiliary speculative drafter; the central outcome is faster serving of the same target model.
...
### 3. [Contraction-Gauge Preconditioning for Quantized Matrix Multiplication](https://arxiv.org/abs/2607.18745) <a id="paper-2607-18745"></a>
...
**Comment:** Product-preserving gauges optimize quantization error while holding the full-precision matrix product fixed.

**Topic Match:** The mechanism concerns quantized matrix products and is demonstrated on an image classifier, without a language-model training-stability result.
...
### 4. [Chebyshev Manifold Adaptation](https://arxiv.org/abs/2607.17377) <a id="paper-2607-17377"></a>
...
### 5. [Visual Token Compression Enhances Robustness of MLLMs](https://arxiv.org/abs/2607.22716) <a id="paper-2607-22716"></a>
...
**Comment:** Distance from the language feature space identifies visual tokens for robustness-oriented pruning.

**Topic Match:** Visual token pruning chiefly addresses jailbreaks and hallucinations during multimodal inference, both outside the requested core subjects.
...
### 6. [MoA-Structured Decode Attention DNF Derivation, KV-Cache Accumulation, GQA/MQA, and OpenACC Kernel](https://arxiv.org/abs/2607.19456) <a id="paper-2607-19456"></a>
...
**Comment:** Algebraic attention derivation removes a transpose buffer and specifies decode-time KV-cache traffic.

**Topic Match:** The artifacts implement existing attention and cache designs for inference, placing the contribution in serving engineering.
