> This is a remedial run for missed papers from 06/26/2026 to 06/28/2026.
> 
...
<sub>Token counts are not reported for this run. 8 of 10 model calls succeeded, 3,483s of model wall clock.</sub>
...
1. [FlexMoE: One-for-All Nested Intra-Expert Pruning for MoE Language Models](#paper-2606-27866)
...
1. [Aurora: A Leverage-Aware Spectral Optimizer](#paper-2606-27715)
**Authors:** Alec Dewulf, Dhruv Pai, Li Yang, Ashley Zhang, Ben Keigwin

2. [On the Nonlinearity of Learning Rate Scaling for LLM Training](#paper-2606-29158)
**Authors:** Zaiwen Yang, Huaqing Zhang, Jing Xu, Jingzhao Zhang

3. [Optimizer Memory Makes Shuffle Order a First-Order Source of Fine-Tuning Noise](#paper-2606-29554)
**Authors:** John Sweeney

4. [Reducing Per-Sample Interference in Stochastic Optimization](#paper-2607-16261)
**Authors:** Apostolos Avranas

5. [Analysis of Adam Algorithms for Stochastic Dynamic Systems](#paper-2606-28879)
**Authors:** Xin Zheng, Yifei Jin, Lei Guo

## [Architecture and Training Dynamics](#topic-architecture-training) (13)

1. [Depth-Staggered Fibonacci Spacing for Sparse Attention: Static Schedules Beat Learned Dilation and Extrapolate Where Dense Attention Fails](#paper-2606-28560)
**Authors:** Chad A. Capps

2. [Flexformer: Flexible Linear Transformer with Learnable Attention Kernel](#paper-2606-27748)
**Authors:** Haoran Zhang, Feng Zhou

3. [Spectral phase transitions and trainability in neural network learning dynamics](#paper-2606-28486)
**Authors:** Chanju Park, Dario Bocchi, Francesco D'Amico, Biagio Lucini, Gert Aarts

4. [MultiHashFormer: Hash-based Generative Language Models](#paper-2606-28057)
**Authors:** Huiyin Xue, Atsuki Yamaguchi, Nikolaos Aletras

5. [Bifocal Diffusion Language Models: Asymmetric Bidirectional Context for Parallel Generation](#paper-2606-27732)
...
6. [Anti-Collapse Dynamics and the Emergence of Multi-Time-Scale Learning in Recurrent Neural Networks](#paper-2606-29519)
**Authors:** Lorenzo Livi

7. [Data Predictability Shapes Weibull Weight-Scale Growth in Transformer Training](#paper-2608-23573)
**Authors:** Tiexin Ding

8. [GLIDE: Guided Layerwise Hybrid Attention for Efficient LLM Inference](#paper-2607-24788)
**Authors:** Vimal William, Ravi Tandon, Jyotikrishna Dass

9. [Syntax vs. Semantics: How Transformers Learn Deep Dependencies](#paper-2608-26139)
**Authors:** Jiangrui Zhao, Xiaoting Du

10. [Class-frequency Guided Noise Schedule for Diffusion Models](#paper-2606-27696)
**Authors:** Jiequan Cui, Beier Zhu, Qingshan Xu, Xiaojuan Qi, Bei Yu, Hanwang Zhang

11. [Adaptive Block Diffusion: Resolving Training-Inference Mismatch in Diffusion Language Models](#paper-2606-29275)
**Authors:** Gagan Jain

12. [Multi-Block Diffusion Language Models](#paper-2606-29215)
**Authors:** Yijie Jin, Jiajun Xu, Yuxuan Liu, Chenkai Xu, Yi Tu, Jiajun Li, Dandan Tu, Xiaohui Yan, Kai Yu, Pengfei Liu, Zhijie Deng

13. [How Width and Data Shape Generalization Scaling Laws in Quadratic Neural Networks](#paper-2606-28242)
...
1. [DLR: Zero-Inference-Cost Latent Residuals for Low-Rank Pre-Training](#paper-2606-28932)
**Authors:** Dong Wang, Wenwu Tang, Yun Cheng, Olga Saukh

2. [High-accuracy Low-Bit KV-Cache Quantization via Local Distribution Restoration](#paper-2607-16248)
**Authors:** Gradwell Dzikanyanga, Yanqi Pan, Weihao Yang, Donglei Wu, Wen Xia, Hao Huang

3. [Layerwise Progressive Freezing: A Training Scaffold for Depth-Scalable Binary Networks](#paper-2606-27759)
...
5. [End-to-End Dynamic Sparsity for Resource-Adaptive LLM Inference](#paper-2606-27743)
...
6. [Depth Exploration for LLM Decoding](#paper-2606-29223)
**Authors:** Weisi Yang, Zipeng Sun, Stephen Xia

7. [BaRA: Bayesian Adaptive Rank Allocation for Parameter-Efficient Fine-Tuning](#paper-2606-29184)
...
### 1. [FlexMoE: One-for-All Nested Intra-Expert Pruning for MoE Language Models](https://arxiv.org/abs/2606.27866) <a id="paper-2606-27866"></a>
...
**Comment:** Ranks expert FFN channels then learns per-expert discrete pruning actions under rising cost pressure, exporting a nested family of MoE subnetworks with one mid-budget recovery fine-tune and runtime budget switching.

**Topic Match:** Intra-expert granularity and MoE compression with kernel-level co-design, centered on MoE structure itself.
...
### 1. [Aurora: A Leverage-Aware Spectral Optimizer](https://arxiv.org/abs/2606.27715) <a id="paper-2606-27715"></a>
...
**Comment:** Identifies arbitrarily non-uniform row norms in Muon updates on tall matrices as a self-reinforcing dead-neuron loop, and enforces row uniformity while preserving the polar-factor geometry.

**Topic Match:** A spectral optimizer for large-scale pretraining whose gains grow with MLP expansion factor; squarely in the optimizer topic.
...
### 2. [On the Nonlinearity of Learning Rate Scaling for LLM Training](https://arxiv.org/abs/2606.29158) <a id="paper-2606-29158"></a>
...
**Comment:** Shows the optimal-LR log-linear scaling law develops upward curvature at scale, and that the curvature vanishes under effective learning rate in normalized weight space with data-side extrapolation.

**Topic Match:** Directly about how large pretraining runs are configured, correcting a scaling law practitioners use to set learning rates.
...
### 3. [Optimizer Memory Makes Shuffle Order a First-Order Source of Fine-Tuning Noise](https://arxiv.org/abs/2606.29554) <a id="paper-2606-29554"></a>
...
**Comment:** Shows fixed-clock optimizer state makes equal-multiset shuffle order an O(eta) rather than O(eta^2) effect, with measured order-variance slopes of 1.83 for AdamW versus 4.00 for SGD.

**Topic Match:** Optimizer-dynamics analysis with direct consequences for how training comparisons are run and seeded.
...
### 4. [Reducing Per-Sample Interference in Stochastic Optimization](https://arxiv.org/abs/2607.16261) <a id="paper-2607-16261"></a>
...
**Comment:** Reformulates the parameter update to explicitly minimize per-sample loss increases, made tractable by reducing to batch dimensionality on the last linear layer, with overhead that shrinks as models grow.

**Topic Match:** An optimizer-level modification that drops into SGD and AdamW, with favourable scaling behaviour.
...
### 5. [Analysis of Adam Algorithms for Stochastic Dynamic Systems](https://arxiv.org/abs/2606.28879) <a id="paper-2606-28879"></a>
...
**Comment:** Adam convergence theory dropping i.i.d. and time-invariant-parameter assumptions, via a Lyapunov function blending both moment recursions.

**Topic Match:** Optimizer analysis yielding hyperparameter guidance, though the setting is stochastic dynamic systems rather than pretraining.
...
### 1. [Depth-Staggered Fibonacci Spacing for Sparse Attention: Static Schedules Beat Learned Dilation and Extrapolate Where Dense Attention Fails](https://arxiv.org/abs/2606.28560) <a id="paper-2606-28560"></a>
...
**Comment:** Matched-recipe study across 21 trained LMs showing a static per-layer stagger of Fibonacci-spaced sparse attention beats learned dilation, and that fixed-offset attention extrapolates to 4x training length where dense collapses.

**Topic Match:** A sparse attention mechanism analysed with a controlled training sweep, including honest negatives on in-length perplexity.
...
### 2. [Flexformer: Flexible Linear Transformer with Learnable Attention Kernel](https://arxiv.org/abs/2606.27748) <a id="paper-2606-27748"></a>
...
**Comment:** Makes random Fourier spectral frequencies trainable so linear attention learns its own kernel family, with a strictly more expressive nonstationary variant and distillation from pretrained softmax transformers.

**Topic Match:** A new attention mechanism that changes subquadratic sequence-modelling expressiveness, not a task application.
...
### 3. [Spectral phase transitions and trainability in neural network learning dynamics](https://arxiv.org/abs/2606.28486) <a id="paper-2606-28486"></a>
...
**Comment:** Casts SGD training as stochastic evolution of a random matrix ensemble, deriving a BBP eigenvalue-detachment transition and a trainability phase diagram in learning rate and init variance.

**Topic Match:** Training-dynamics analysis connecting optimization hyperparameters to trainability.
...
### 4. [MultiHashFormer: Hash-based Generative Language Models](https://arxiv.org/abs/2606.28057) <a id="paper-2606-28057"></a>
...
**Comment:** Replaces the vocabulary-linear embedding matrix with multi-hash signatures plus hash encoder/decoder, enabling causal LM under collisions and constant-footprint vocabulary expansion.

**Topic Match:** A new architectural mechanism for token representation, validated in pretraining at up to 3B.
...
### 5. [Bifocal Diffusion Language Models: Asymmetric Bidirectional Context for Parallel Generation](https://arxiv.org/abs/2606.27732) <a id="paper-2606-27732"></a>
...
**Comment:** Resolves the bidirectional-attention versus KV-cache dilemma in diffusion LMs by pairing causal attention with a lightweight reverse Mamba sidecar for compressed right context, validated on 60B-token continued pretraining.

**Topic Match:** A new architectural mechanism combining attention and state-space modelling, trained at meaningful scale.
...
### 6. [Anti-Collapse Dynamics and the Emergence of Multi-Time-Scale Learning in Recurrent Neural Networks](https://arxiv.org/abs/2606.29519) <a id="paper-2606-29519"></a>
...
**Comment:** Derives a threshold at which heavy-tailed learning-dynamics fluctuations sustain power-law rather than exponential forgetting, with one spectral exponent governing both time-scale spread and memory decay.

**Topic Match:** Training-dynamics theory explaining when recurrent sequence models can learn long-range dependence at all.
...
### 7. [Data Predictability Shapes Weibull Weight-Scale Growth in Transformer Training](https://arxiv.org/abs/2608.23573) <a id="paper-2608-23573"></a>
...
**Comment:** Training-free bigram-entropy statistic predicts Weibull weight-scale growth across learning rates, a forward predictor of training dynamics.

**Topic Match:** Core contribution is an empirical law linking data statistics to weight-norm evolution during transformer training.
...
### 8. [GLIDE: Guided Layerwise Hybrid Attention for Efficient LLM Inference](https://arxiv.org/abs/2607.24788) <a id="paper-2607-24788"></a>
...
**Comment:** Exploits layer-wise heterogeneity in softmax sensitivity to give each layer a variable-size sliding window plus linear recurrence, non-uniformly compressing the KV footprint.

**Topic Match:** A hybrid attention/recurrence architectural mechanism whose payoff is KV-cache efficiency.
...
### 9. [Syntax vs. Semantics: How Transformers Learn Deep Dependencies](https://arxiv.org/abs/2608.26139) <a id="paper-2608-26139"></a>
...
**Comment:** Identifies gradient starvation suppressing sparse semantic-dependency signals early in optimization, explaining the phase-transition emergence of structural reasoning and why CoT bypasses it.

**Topic Match:** Optimization-dynamics account of what transformers learn when, validated from toy models to 7-8B production models.
...
### 10. [Class-frequency Guided Noise Schedule for Diffusion Models](https://arxiv.org/abs/2606.27696) <a id="paper-2606-27696"></a>
...
**Comment:** Class-frequency-dependent noise schedules address rare-class score-estimation errors in diffusion training.

**Topic Match:** The core contribution connects class imbalance to diffusion training dynamics and changes noise scheduling accordingly, giving a focused training-mechanism contribution.
...
### 11. [Adaptive Block Diffusion: Resolving Training-Inference Mismatch in Diffusion Language Models](https://arxiv.org/abs/2606.29275) <a id="paper-2606-29275"></a>
...
**Comment:** Trains diffusion LMs over a distribution of prefix-window configurations to remove the fixed-block train/inference mismatch, with a support-coverage argument for generalization across decoding policies.

**Topic Match:** Core contribution is a training-objective/dynamics change in a sequence-modelling architecture, requiring no architectural edit.
...
### 12. [Multi-Block Diffusion Language Models](https://arxiv.org/abs/2606.29215) <a id="paper-2606-29215"></a>
...
**Comment:** Multi-block teacher forcing post-trains block diffusion LMs on bounded noise-groups matching multi-block inference states, with a block buffer that keeps shapes static and prefix caches reusable.

**Topic Match:** A training-objective fix paired with a decoding schedule, translating parallelism into measured wall-clock gains.
...
### 13. [How Width and Data Shape Generalization Scaling Laws in Quadratic Neural Networks](https://arxiv.org/abs/2606.28242) <a id="paper-2606-28242"></a>
...
**Comment:** Exact characterization of generalization error jointly in width, samples, and regularization for a feature-learning quadratic network, yielding a phase diagram of scaling regimes.

**Topic Match:** Scaling-law theory beyond fixed-feature and infinite-width regimes, though in a two-layer toy model.
...
### 1. [DLR: Zero-Inference-Cost Latent Residuals for Low-Rank Pre-Training](https://arxiv.org/abs/2606.28932) <a id="paper-2606-28932"></a>
...
**Comment:** A parameter-free structured residual that replicates latent coordinates during low-rank pretraining and folds exactly into the up-projection afterwards, closing part of the gap to full-rank training at zero deployment cost.

**Topic Match:** Directly attacks the cost of pretraining large models via a low-rank mechanism, validated from 60M to 7B.
...
### 2. [High-accuracy Low-Bit KV-Cache Quantization via Local Distribution Restoration](https://arxiv.org/abs/2607.16248) <a id="paper-2607-16248"></a>
...
**Comment:** Diagnoses 1-bit KV-cache failure as structured top-K misranking rather than logit error, and restores only the at-risk local distribution.

**Topic Match:** New mechanism for low-bit KV-cache quantization, squarely in memory-efficiency work.
...
### 3. [Layerwise Progressive Freezing: A Training Scaffold for Depth-Scalable Binary Networks](https://arxiv.org/abs/2606.27759) <a id="paper-2606-27759"></a>
...
**Comment:** Replaces STE with layerwise progressive binarization and traces depth collapse to activation-induced gradient blockades, showing progression order is decisive.

**Topic Match:** A new quantization-training mechanism with a mechanistic account of why deep binary nets fail to train.
...
**Comment:** Links eviction-induced degradation to lost unique-token coverage and bounds it via mutual information, then retains tokens across heads and layers accordingly.

**Topic Match:** KV-cache memory efficiency with a new coverage criterion replacing attention-sparsity heuristics.
...
### 5. [End-to-End Dynamic Sparsity for Resource-Adaptive LLM Inference](https://arxiv.org/abs/2606.27743) <a id="paper-2606-27743"></a>
...
**Comment:** Budget-conditioned gates trained end-to-end so one model traces the whole compute-accuracy frontier via layer skipping, head pruning, and reasoning-token reduction.

**Topic Match:** Dynamic computation conditioned on runtime budget, a sparsity mechanism rather than deployment tuning.
...
### 6. [Depth Exploration for LLM Decoding](https://arxiv.org/abs/2606.29223) <a id="paper-2606-29223"></a>
...
**Comment:** Replaces single early-exit depth selection with parallel exploration over candidate depths, committing only final-depth tokens and collapsing the lattice to keep decoding losslessly equivalent.

**Topic Match:** Exploits the depth axis as a dynamic-computation mechanism, though the payoff is decoding throughput rather than training cost.
...
### 7. [BaRA: Bayesian Adaptive Rank Allocation for Parameter-Efficient Fine-Tuning](https://arxiv.org/abs/2606.29184) <a id="paper-2606-29184"></a>
...
**Comment:** Bayesian gating over disentangled latent factors gives instance-wise effective rank, with a generalization bound depending on joint effective rank rather than max rank.

**Topic Match:** Adaptive-rank low-rank adaptation with a supporting complexity analysis.
