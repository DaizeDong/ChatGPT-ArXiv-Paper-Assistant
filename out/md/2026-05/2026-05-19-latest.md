...
2. [AMO: Adaptive Muon Orthogonalization](#paper-2605-17806)
...
3. [Continuous Diffusion Scales Competitively with Discrete Diffusion for Language](#paper-2605-18530)
**Authors:** Zhihan Yang, Wei Guo, Shuibai Zhang, Subham Sekhar Sahoo, Yongxin Chen, Arash Vahdat, Morteza Mardani, John Thickstun

4. [A Geometric Analysis of Sign-Magnitude Asymmetry in a ReLU + RMSNorm Block under Ternary Quantization](#paper-2605-18933)
**Authors:** Lei Dong

5. [DashAttention: Differentiable and Adaptive Sparse Hierarchical Attention](#paper-2605-18753)
...
7. [Flash PD-SSM: Memory-Optimized Structured Sparse State-Space Models](#paper-2605-19150)
**Authors:** Aleksandar Terzić, Francesco Carzaniga, Nicolas Menet, Yannick Biehl, Michael Hersche, Thomas Hofmann, Abbas Rahimi

8. [Distance-Aware Muon: Adaptive Step Scaling for Normalized Optimization](#paper-2605-18999)
**Authors:** Yury Demidovich, Abhishek Chakraborty, Grigory Malinovsky, Angelia Nedić, Peter Richtárik

9. [InfoFlow: A Framework for Multi-Layer Transformer Analysis](#paper-2605-17930)
**Authors:** Penghao Yu, Haotian Jiang, Zeyu Bao, Qianxiao Li

10. [Multi-Head Attention as Ensemble Nadaraya-Watson Estimation: Variance Reduction, Decorrelation, and Optimal Head Diversity](#paper-2605-20271)
**Authors:** Ernest Fokoué

11. [Forward-Learned Discrete Diffusion: Learning how to noise to denoise faster](#paper-2605-18204)
**Authors:** Grigory Bartosh, Teodora Pandeva, Sushrut Karmalkar, Javier Zazo

12. [A More Word-like Image Tokenization for MLLMs](#paper-2605-17954)
**Authors:** Hyun Lee, Hyemin Jeong, Yejin Kim, Hyungwook Choi, Hyunsoo Cho, Soo Kyung Kim, Joonseok Lee

13. [Scale-Invariant Neural Network Optimization: Norm Geometry and Heavy-Tailed Noise](#paper-2605-18528)
**Authors:** Jiayu Zhang, Tianyi Lin

14. [Perfect Parallelization in Mini-Batch SGD with Classical Momentum Acceleration](#paper-2605-18609)
**Authors:** Sachin Garg, Michał Dereziński

15. [Can Adaptive Gradient Methods Converge under Heavy-Tailed Noise? A Case Study of AdaGrad](#paper-2605-18694)
**Authors:** Zijian Liu

16. [Graph Hierarchical Recurrence for Long-Range Generalization](#paper-2605-18387)
**Authors:** Stefano Carotti, Marco Pacini, Alessio Gravina, Davide Bacciu, Bruno Lepri, Sebastiano Bontorin

17. [Divergence-Suppressing Couplings for Rectified Flow](#paper-2605-17733)
...
3. [TierCheck: Tiered Checkpointing for Fault Tolerance in Large Language Model Training](#paper-2605-17821)
**Authors:** Shujie Han, Feng Jiang, Patrick P. C. Lee, Xiao Zhang, Zhijie Huang, Nannan Zhao, Xiaonan Zhao, Lichen Pan

4. [Guard: Scalable Straggler Detection and Node Health Management for Large-Scale Training](#paper-2605-17879)
...
5. [$\boldsymbol{f}$-OPD: Stabilizing Long-Horizon On-Policy Distillation with Freshness-Aware Control](#paper-2605-17862)
**Authors:** Xianwei Chen, Shimin Zhang, Jibin Wu

## [MoE Where It Changes the Design Space](#topic-moe-training) (1)

1. [CoX-MoE: Coalesced Expert Execution for High-Throughput MoE Inference with AMX-Enabled CPU-GPU Co-Execution](#paper-2605-17889)
...
1. [Protection Is (Nearly) All You Need: Structural Protection Dominates Scoring in Globally Capped KV Eviction](#paper-2605-18053)
**Authors:** Gabriel Garcia

2. [GAMMA: Global Bit Allocation for Mixed-Precision Models under Arbitrary Budgets](#paper-2605-18475)
**Authors:** Zhangyang Yao, Haiyan Zhao, Haoyu Wang, Xu Han

3. [KVBuffer: IO-aware Serving for Linear Attention](#paper-2605-19049)
**Authors:** Longwei Zou, Lin Zhong

4. [Prune, Update and Trim: Robust Structured Pruning for Large Language Models](#paper-2605-18331)
**Authors:** Diego Coello de Portugal Mecke, Tom Hanika, Lars Schmidt-Thieme

5. [Learning When to Adapt](#paper-2605-19028)
**Authors:** Ali Zindari, Xiaowen Jiang, Rotem Mulayoff, Sebastian U. Stich

6. [MARR: Module-Adaptive Residual Reconstruction for Low-Bit Post-Training Quantization](#paper-2605-17997)
**Authors:** Le Su, Xing Luo, Zhi Jin

7. [LoRA vs. Full Fine-Tuning: A Theoretical Perspective](#paper-2605-19018)
**Authors:** Ali Zindari, Rotem Mulayoff, Sebastian U. Stich

8. [Focused Forcing: Content-Aware Per-Frame KV Selection for Efficient Autoregressive Video Diffusion](#paper-2605-18346)
...
**Comment:** Shows how output-gradient imbalance and spikes restrict SGD learning rates, then tests clipping at 1B parameters.

**Topic Match:** Directly investigates the pretraining dynamics behind Adam's advantage; the remaining 3.5% loss gap is demonstrated at a 1M-token batch.
...
### 2. [AMO: Adaptive Muon Orthogonalization](https://arxiv.org/abs/2605.17806) <a id="paper-2605-17806"></a>
...
**Comment:** Links matrix geometry to Muon orthogonalization difficulty and allocates Newton-Schulz iterations by operator type.

**Topic Match:** Explains heterogeneity within a pretraining optimizer and tests a geometry-informed schedule on two billion-parameter model families.
...
### 3. [Continuous Diffusion Scales Competitively with Discrete Diffusion for Language](https://arxiv.org/abs/2605.18530) <a id="paper-2605-18530"></a>
...
### 4. [A Geometric Analysis of Sign-Magnitude Asymmetry in a ReLU + RMSNorm Block under Ternary Quantization](https://arxiv.org/abs/2605.18933) <a id="paper-2605-18933"></a>
...
**Comment:** Holds perturbation norm fixed to explain sign sensitivity through RMSNorm geometry and measurable outlier amplification.

**Topic Match:** Analyzes what normalization and activation geometry compute, tests quantitative predictions on TinyLlama, and explicitly rejects unsupported multilayer compounding.
...
### 5. [DashAttention: Differentiable and Adaptive Sparse Hierarchical Attention](https://arxiv.org/abs/2605.18753) <a id="paper-2605-18753"></a>
...
**Comment:** Replaces hard top-k block selection with query-adaptive entmax, making hierarchical attention differentiable.

**Topic Match:** The core change is the trainable attention-selection operator and gradient path; named baselines are provided, but model scale and datasets are unspecified.
...
**Comment:** Connects depth-wise normalization to attention-sink amplification and modifies residual routing with a null channel.

**Topic Match:** The core contribution explains and modifies the normalization and residual-routing mechanism that produces sinks and activation outliers.
...
### 7. [Flash PD-SSM: Memory-Optimized Structured Sparse State-Space Models](https://arxiv.org/abs/2605.19150) <a id="paper-2605-19150"></a>
...
**Comment:** Uses discrete selection among structured sparse transition matrices to recover unstructured-state expressivity.

**Topic Match:** Changes the recurrent transition structure and reports hybrid-LLM experiments, although the language-model scales and baselines are unnamed.
...
### 8. [Distance-Aware Muon: Adaptive Step Scaling for Normalized Optimization](https://arxiv.org/abs/2605.18999) <a id="paper-2605-18999"></a>
...
**Comment:** Adapts Muon's normalized step scale using trajectory distances and local descent certificates.

**Topic Match:** Directly addresses optimizer parameterization and scale sensitivity, with a small language-model demonstration.
...
### 9. [InfoFlow: A Framework for Multi-Layer Transformer Analysis](https://arxiv.org/abs/2605.17930) <a id="paper-2605-17930"></a>
...
**Comment:** Separates depth from parameter cost at fixed retrieval precision through explicit information-propagation mechanisms.

**Topic Match:** Explains architectural consequences of stacking attention layers through retrieval and information-decoding mechanisms.
...
### 10. [Multi-Head Attention as Ensemble Nadaraya-Watson Estimation: Variance Reduction, Decorrelation, and Optimal Head Diversity](https://arxiv.org/abs/2605.20271) <a id="paper-2605-20271"></a>
...
**Comment:** Derives a head-diversity error criterion and optimal head allocation while holding total attention dimension fixed.

**Topic Match:** Addresses how attention-head subspaces determine estimation error and the architectural tradeoff between head count and width.
...
### 11. [Forward-Learned Discrete Diffusion: Learning how to noise to denoise faster](https://arxiv.org/abs/2605.18204) <a id="paper-2605-18204"></a>
...
**Comment:** Replaces fixed Markovian noising with a learned process while holding the reverse parameterization and sampling-step count fixed.

**Topic Match:** Learned noising changes a standard generative-model primitive, but the abstract identifies no language-model training setting or named experimental benchmark.
...
### 12. [A More Word-like Image Tokenization for MLLMs](https://arxiv.org/abs/2605.17954) <a id="paper-2605-17954"></a>
...
**Comment:** Replaces fixed image-patch tokens with semantic clusters whose number adapts to image complexity.

**Topic Match:** Closest to token-interface design, but the trained component is a visual projector feeding a fixed language model.
...
### 13. [Scale-Invariant Neural Network Optimization: Norm Geometry and Heavy-Tailed Noise](https://arxiv.org/abs/2605.18528) <a id="paper-2605-18528"></a>
...
**Comment:** Characterizes dimension-dependent limits of spectral-norm optimization under heavy-tailed gradient noise.

**Topic Match:** Optimizer geometry is adjacent, but the core contribution is general stochastic-gradient complexity theory without a specified language-model pretraining result.
...
### 14. [Perfect Parallelization in Mini-Batch SGD with Classical Momentum Acceleration](https://arxiv.org/abs/2605.18609) <a id="paper-2605-18609"></a>
...
**Comment:** Derives how classical momentum acceleration increases with minibatch size until saturation.

**Topic Match:** Closest to optimizer dynamics, but the contribution is excluded stochastic-gradient analysis on interpolating quadratics.
...
### 15. [Can Adaptive Gradient Methods Converge under Heavy-Tailed Noise? A Case Study of AdaGrad](https://arxiv.org/abs/2605.18694) <a id="paper-2605-18694"></a>
...
**Comment:** Establishes conditions under which unmodified AdaGrad converges despite heavy-tailed gradient noise.

**Topic Match:** Closest to optimizer dynamics, but the core is excluded stochastic-gradient convergence analysis without a language-training setting.
...
### 16. [Graph Hierarchical Recurrence for Long-Range Generalization](https://arxiv.org/abs/2605.18387) <a id="paper-2605-18387"></a>
...
**Comment:** Combines recurrence over an input graph and its pooled hierarchy to extend distance generalization.

**Topic Match:** Recurrence is an architectural mechanism, but the models and tasks are graph learning rather than language-model training.
...
### 17. [Divergence-Suppressing Couplings for Rectified Flow](https://arxiv.org/abs/2605.17733) <a id="paper-2605-17733"></a>
...
**Comment:** Suppresses velocity-field divergence during coupling generation to improve subsequent flow training.

**Topic Match:** Closest to generative training dynamics, but the contribution concerns synthetic distributions and image generation.
...
**Comment:** Extends stale-gradient rejection to asynchronous LMO-based momentum updates and tests language-model pretraining.

**Topic Match:** Changes the optimization algorithm's treatment of asynchronous gradients, with NanoChat experiments connecting the theory to language-model training.
...
### 3. [TierCheck: Tiered Checkpointing for Fault Tolerance in Large Language Model Training](https://arxiv.org/abs/2605.17821) <a id="paper-2605-17821"></a>
...
**Comment:** Combines memory-resident differential checkpoints with persistent base checkpoints under global consistency.

**Topic Match:** Checkpointing belongs to training infrastructure, but the contribution improves persistence and recovery rather than the distributed optimization algorithm.
...
### 4. [Guard: Scalable Straggler Detection and Node Health Management for Large-Scale Training](https://arxiv.org/abs/2605.17879) <a id="paper-2605-17879"></a>
...
**Comment:** Combines online straggler detection with systematic node qualification for large training clusters.

**Topic Match:** Training infrastructure is the nearest category; the contribution improves operational reliability and utilization without changing the training algorithm.
...
### 5. [$\boldsymbol{f}$-OPD: Stabilizing Long-Horizon On-Policy Distillation with Freshness-Aware Control](https://arxiv.org/abs/2605.17862) <a id="paper-2605-17862"></a>
...
**Comment:** Separates rollout drift from teacher-context drift to control the influence of stale training samples.

**Topic Match:** Asynchronous objective control is the nearest fit, but the core target is agentic post-training through on-policy distillation.
...
### 1. [CoX-MoE: Coalesced Expert Execution for High-Throughput MoE Inference with AMX-Enabled CPU-GPU Co-Execution](https://arxiv.org/abs/2605.17889) <a id="paper-2605-17889"></a>
...
**Comment:** Coalesces expert execution and distributes inference workloads across AMX-enabled CPUs and GPUs.

**Topic Match:** MoE is the nearest subject category, but expert execution and offloading are explicitly excluded inference systems work.
...
### 1. [Protection Is (Nearly) All You Need: Structural Protection Dominates Scoring in Globally Capped KV Eviction](https://arxiv.org/abs/2605.18053) <a id="paper-2605-18053"></a>
...
**Comment:** Isolates boundary-token protection from eviction scoring through controlled comparisons and equivalence tests.

**Topic Match:** Closest to cache compression, but the core subject is long-context inference and KV eviction.
...
### 2. [GAMMA: Global Bit Allocation for Mixed-Precision Models under Arbitrary Budgets](https://arxiv.org/abs/2605.18475) <a id="paper-2605-18475"></a>
...
### 3. [KVBuffer: IO-aware Serving for Linear Attention](https://arxiv.org/abs/2605.19049) <a id="paper-2605-19049"></a>
...
**Comment:** Buffers keys and values to amortize recurrent-state memory traffic during linear-attention serving.

**Topic Match:** Memory efficiency is the nearest topic, but the contribution improves serving latency and request capacity without changing training.
...
### 4. [Prune, Update and Trim: Robust Structured Pruning for Large Language Models](https://arxiv.org/abs/2605.18331) <a id="paper-2605-18331"></a>
...
**Comment:** Combines sequential FFN error compensation with attention-head pruning, including grouped-query attention.

**Topic Match:** Structured pruning fits the category broadly, but the method combines established post-training steps without establishing an unconditional training-enabling gain.
...
### 5. [Learning When to Adapt](https://arxiv.org/abs/2605.19028) <a id="paper-2605-19028"></a>
...
**Comment:** Uses input-dependent gates over rank-one LoRA components to limit unwanted task updates.

**Topic Match:** The nearest topic is low-rank parameterization, but the core contribution is a parameter-efficient fine-tuning mechanism for reducing forgetting.
...
### 6. [MARR: Module-Adaptive Residual Reconstruction for Low-Bit Post-Training Quantization](https://arxiv.org/abs/2605.17997) <a id="paper-2605-17997"></a>
...
**Comment:** Tunes residual reconstruction strength per module using feedback from quantization error.

**Topic Match:** Quantization is the nearest category, but this refines an existing post-training reconstruction method rather than enabling new training behavior.
...
### 7. [LoRA vs. Full Fine-Tuning: A Theoretical Perspective](https://arxiv.org/abs/2605.19018) <a id="paper-2605-19018"></a>
...
**Comment:** Relates LoRA's excess-risk advantage to low-rank shifts between pretraining and downstream tasks.

**Topic Match:** Low-rank methods are the nearest category, but the subject is parameter-efficient downstream fine-tuning, with the main theory in linear regression.
...
### 8. [Focused Forcing: Content-Aware Per-Frame KV Selection for Efficient Autoregressive Video Diffusion](https://arxiv.org/abs/2605.18346) <a id="paper-2605-18346"></a>
...
**Comment:** Selects cached video history separately for each generated frame and attention head.

**Topic Match:** Closest to KV-cache efficiency, but the method targets video-generation inference.
