...
| Phase 6 | convert daily hotspot sections into sortable table feeds and split daily topics into its own module | completed |
| Phase 7 | align the daily header with the paper page and add a shared floating light/dark toggle | completed |
| Phase 8 | tighten dense table presentation across daily and detail pages, then run full validation | completed |

## Current Focus

The current redesign iteration moves the hotspot UI from a compact feed of stacked rows into a true table-first information board.

The main requirements are:

- each source-family section should be a sortable table
- each row should use a stable schema:
  - `Title`
  - `Topic`
  - `Source`
  - `Signal`
  - `Score`
  - `Heat`
- the daily topic map should be a dedicated module, not merged into the top summary area
- the top summary bar should become visually closer to the paper digest page: larger date navigation, minimal framing, and no box background
- the hotspot frontend should reuse the floating theme toggle model from the paper site

Current result:

- the daily hotspot route is now table-first and sortable section-by-section
- daily topics are separated into their own sortable module
- the top summary bar is visually closer to the paper page and no longer uses a boxed hero treatment
- duplicate `Index` / `Hotspot index` links inside the same interface were removed in favor of a single global brand/home path
- the hotspot web app now uses the same floating light/dark toggle behavior as the paper site
...
The current implementation adds a confidence-aware post-filter after LLM screening:

- single-source, low-evidence community or tooling candidates can still survive in the broader feed
- but they are demoted out of `Featured Topics` unless they are needed to satisfy the minimum featured count
- this keeps the main strip more trustworthy without deleting same-day coverage
...
### 7.4 Confidence-aware screening and API utilization

The hotspot pipeline now uses a four-way pre-LLM split before any OpenAI screening call:

- `auto_keep`: high-confidence, high-evidence, strong multi-source clusters
- `auto_drop`: weak low-evidence noise
- `heuristic_only`: low-confidence single-source repo/discussion tails that should stay visible in radar/long-tail but do not justify LLM review
- `review`: the remaining ambiguous candidates

Only the `review` bucket is sent to the screening model.

Additional API-efficiency rules:

- the review queue is capped dynamically based on the daily featured/watchlist targets instead of always using the maximum configured review budget
- screening batches are larger to reduce duplicated system/prompt overhead
- per-cluster screening prompts use fewer representative items and shorter summaries
- digest synthesis caps per-topic evidence items and drops URLs from the synthesis payload because they are not needed for writing the short summary

This preserves broad coverage in the source-first UI while concentrating LLM spend on ambiguous high-value candidates instead of obvious tails.
