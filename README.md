...
4. **Pick a runner that can reach a model.** The keyless backends are both local programs: `llmcall` is not a PyPI dependency of this repo, and the repo-local fallback shells out to the `claude` CLI. A stock GitHub-hosted runner has neither and cannot log either of them in, so on `ubuntu-latest` the default `[LLM] backend = auto` resolves to the agent transport, finds no CLI, and every scoring call fails -- loudly, by design, but the digest for that day is empty. Three ways out, in the order they are worth considering:
   - **Self-hosted runner** (recommended): run the workflow on a machine where `llmcall` or the `claude` CLI is already logged in. Change `runs-on: ubuntu-latest` to `runs-on: self-hosted` and nothing else moves. This is the only option that keeps the zero-key property.
   - **Run the pipeline off Actions entirely** (cron on that machine or a VPS) and leave Actions as the publisher, which is already how hotspots are split (`[HOTSPOT_RUNTIME] runtime`).
   - **Legacy OpenAI path for CI only**: set `[LLM] backend = openai` and provide `OPENAI_API_KEY` (+ `OPENAI_BASE_URL`) as [GitHub Secrets](https://docs.github.com/en/actions/security-guides/using-secrets-in-github-actions); see [GUIDE_GITHUB_API.md](GUIDE_GITHUB_API.md) for a free option. This one costs money and reintroduces a key to rotate.

   Do **not** try to make the keyless chain work on a hosted runner by putting CLI credentials into Actions secrets: those are personal subscription credentials, and a hosted runner is not a place to send them.
